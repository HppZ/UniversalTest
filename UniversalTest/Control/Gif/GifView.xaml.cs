    ...
    public sealed partial class GifView : UserControl
    {
        ...
        private class GifPresenter
        {
            ...
            private async Task AdvanceFrame()
            {
                ...
                try
                {
                    PrepareFrame(pixels, frameRectangle, disposeRectangle, frameIndex);
            ...
            private void PrepareFrame(byte[] pixels, Rect frameRectangle, Rect disposeRectangle, int framecount)
            {
                var sharedDevice = GetSharedDevice();
                ...
                using (frameBitmap)
                {
                    if (!disposeRectangle.IsEmpty || framecount == 0)
                    {
                        var imageRectangle = new Rect(new Point(), _canvasImageSource.Size);
                        using (drawingSession.CreateLayer(1.0f, imageRectangle))
                        {
